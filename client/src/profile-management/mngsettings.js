...
function ManageSettings() {
  ...
  const updateStatus = () => {
    ...
    if (AnonymusStatReg == "") {
      AnonymusStatVal = "0";
    } else if (AnonymusStatReg == "Y") {
      AnonymusStatVal = "1";
    } else {
      alert("Error");
    }

    Axios.put("http://localhost:4000/api/mngsettings/status", {
      AnonymusStat: AnonymusStatVal,
  ...
  const updateAddress = () => {
    Axios.put("http://localhost:4000/api/mngsettings/address", {
      Address: AddressReg,
      City: CityReg,
      State: StateReg,
      ZipCode: ZipCodeReg,
