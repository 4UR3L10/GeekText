...
router.put("/fullname", (req, res) => {
    ...
    if (UserFirstName && UserLastName) {
        try {
            db.promise().query(`UPDATE user SET user_firstname = '${UserFirstName}', user_lastname = '${UserLastName}'  WHERE email = '${IdEmail}'`);
...
router.put("/email", (req, res) => {
    ...
    if (EmailAddress) {
        try {
            db.promise().query(`UPDATE user SET email = '${EmailAddress}' WHERE email = '${IdEmail}'`);
...
router.put("/nickname", (req, res) => {
    ...
    if (NickName) {
        try {
            db.promise().query(`UPDATE user SET nickname = '${NickName}' WHERE email = '${IdEmail}'`);
...
router.put("/status", (req, res) => {
    ...
    // if (AnonymusStat) {
    try {
        db.promise().query(`UPDATE user SET AnonymusStat = '${AnonymusStat}'  WHERE EmailAddress = '${IdEmail}'` // CHANGE TO DYNAMIC USERRRRRR
        );
...
router.put("/password", (req, res) => {
    ...
    if (Password) {
        try {
            db.promise().query(`UPDATE user SET Password = '${Password}' WHERE email = '${IdEmail}'` // CHANGE TO DYNAMIC USERRRRRR
            );
        ...
        try {
            db.query(`SELECT UserID FROM user WHERE EmailAddress = '${IdEmail}'`, (error, results) => {
                if (results == "") {
                    ...
                    db.promise().query(`UPDATE home_address SET Address = '${Address}',Address2 = '${Address2}',City = '${City}',State = '${State}',ZipCode = '${ZipCode}',Country = '${Country}', HmAddrssVld = 'N' WHERE UserID = '${
                        results[0].UserID
                    }'`);
